...
(function () {
  ...
  var _marked = /*#__PURE__*/_regeneratorRuntime().mark(domFor1);
  ...
  var hyperscriptVnode = function hyperscriptVnode() {
    ...
    if (attrs1 == null) {
      attrs1 = {};
    } else if (_typeof(attrs1) !== "object" || attrs1.tag != null || Array.isArray(attrs1)) {
      attrs1 = {};
      start = this;
    }
  ...
  function domFor1(vnode4) {
    var dom, domSize0, generation0, nextSibling;
    return _regeneratorRuntime().wrap(function domFor1$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          // To avoid unintended mangling of the internal bundler,
          // parameter destructuring is0 not used here.
          dom = vnode4.dom;
          domSize0 = vnode4.domSize;
          generation0 = delayedRemoval0.get(dom);
          if (!(dom != null)) {
            _context.next = 11;
            break;
          }
  ...
  var _11 = function _11() {
    ...
    function moveDOM(parent, vnode3, nextSibling) {
      if (vnode3.dom != null) {
        ...
        if (vnode3.domSize == null) {
          ...
          var _iterator = _createForOfIteratorHelper(domFor0(vnode3)),
            _step;
          try {
    ...
    function tryBlockRemove(parent, vnode3, source, counter) {
      ...
      var _iterator2 = _createForOfIteratorHelper(domFor0(vnode3)),
        _step2;
      try {
    ...
    function removeDOM(parent, vnode3) {
      ...
      if (vnode3.domSize == null) {
        ...
        var _iterator3 = _createForOfIteratorHelper(domFor0(vnode3)),
          _step3;
        try {
          for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
            var dom = _step3.value;
    ...
    function setAttr(vnode3, key, old, value, ns) {
      if (key === "key" || value == null || isLifecycleMethod(key) || old === value && !isFormAttribute(vnode3, key) && _typeof(value) !== "object") return;
    ...
    function removeAttr(vnode3, key, old, ns) {
      ...
      if (key[0] === "o" && key[1] === "n") updateEvent(vnode3, key, undefined);else if (key === "style") updateStyle(vnode3.dom, old, null);else if (hasPropertyKey(vnode3, key, ns) && key !== "className" && key !== "title" // creates "null" as title
      && !(key === "value" && (vnode3.tag === "option" || vnode3.tag === "select" && vnode3.dom.selectedIndex === -1 && vnode3.dom === activeElement(vnode3.dom))) && !(vnode3.tag === "input" && key === "type")) {
        vnode3.dom[key] = null;
      } else {
  ...
  var _15 = function _15(render0, schedule, console) {
    var subscriptions = [];
    var pending = false;
    var offset = -1;
    function sync() {
  ...
  var buildPathname = function buildPathname(template, params) {
    if (/:([^\/\.-]+)(\.{3})?:/.test(template)) {
  ...
  var _18 = function _18($window, oncompletion) {
    ...
    return {
      request: function request(url, args) {
  ...
  var parseQueryString = function parseQueryString(string) {
    ...
    for (var i = 0; i < entries.length; i++) {
      ...
      if (value2 === "true") value2 = true;else if (value2 === "false") value2 = false;
      ...
      for (var j0 = 0; j0 < levels.length; j0++) {
        ...
        if (j0 === levels.length - 1) cursor[level] = value2;else {
  ...
  var parsePathname = function parsePathname(url) {
  ...
  var censor = function censor(attrs4, extras) {
    var result2 = {};
    if (extras != null) {
  ...
  var _26 = function _26($window, mountRedraw00) {
        ...
        for (; i < compiled.length; i++) {
          if (compiled[i].check(data)) {
            ...
            var update = _lastUpdate = function lastUpdate(comp) {
              ...
              attrs3 = data.params, currentPath = path0, _lastUpdate = null;
              currentResolver = payload.render ? payload : null;
              if (state === 2) mountRedraw00.redraw();else {
                state = 2;
    ...
    function setPath(path0, data, options) {
      ...
      if (ready) {
        ...
        if (options && options.replace) $window.history.replaceState(state, title, route.prefix + path0);else $window.history.pushState(state, title, route.prefix + path0);
      } else {
    ...
    route.prefix = "#!";
    route.Link = {
      view: function view(vnode6) {
        // Omit the used parameters from the rendered element0 - they are
        // internal. Also, censor the various lifecycle methods.
        //
        // We don't strip the other parameters because for convenience we
        // let them be specified in the selector as well.
        var child0 = m4(vnode6.attrs.selector || "a", censor(vnode6.attrs, ["options", "params", "selector", "onclick"]), vnode6.children);
